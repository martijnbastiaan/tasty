...
createTestActions opts0 tree = do
  ...
  where
    ...
    foldGroup :: OptionSet -> ExecutionMode -> TestName -> [Tr] -> Tr
    foldGroup _opts Parallel name trees =
      TGroup <$> local (first (|> name)) (sequence trees)
    foldGroup _opts (Sequential depType) name trees =
      TGroup <$>
        local
          (first (|> name))
